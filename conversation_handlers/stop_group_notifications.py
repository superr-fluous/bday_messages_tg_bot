...
from helpers.bot_types import GROUP_INFO_ROW
from db_manage.db_helpers import get_group_row_by_id, get_group_row_by_name
...
async def start_stop_group_notification(update: Update, context: CallbackContext) -> int:
  """Начало диалога """
  await update.message.reply_text("Введи id или название группы (для отмены напиши /cancel)")

  return stop_group_notifications_stages['step_1']

async def spgn_group_id_name(update: Update, context: CallbackContext) -> int | str:
  ...
  trigger_group_notifications(context, int(group[0]), 'stop')
